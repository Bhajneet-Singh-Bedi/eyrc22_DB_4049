...
function [x_1,x_2] = find_equilibrium_points(x1_dot, x2_dot)
  ...
  ## For finding equlilibrium points -> means calculating various values of x1 and x2 by using various methods (in OCTAVE)
  ## Use the function solve for that
  ## [x1_sol x2_sol] = solve(eq1, eq1)
...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  solutions = [x_1, x_2];
  jacobian_matrices = {};
  # Jac matrix using jacobian fucntion
  jac = jacobian([x1_dot x2_dot])
  # Substituting respective values of eq points in jac
  # and storing it in jacobian_matrices for further use.
  for i=1:length(solutions)
    #jacobian_matrices(i)=double(subs(jac, [x1, x2],solutions(i,:))); 

...
function dy = pendulum_dynamics(y, m, L, g, u)
  ...
  cos_theta = cos(y(1));
  dy(1,1) = y(2);                               
  dy(2,1) = (-(g/L)*sin_theta) + (u/(m*(L^2)));## This is -g/l(sin(x1)) + (1/ml^2)*u, NOTE: add . in front of arithmatic operators to represent scalar multiplication.    
endfunction
...
function [t,y] = pole_place_pendulum(m, g, L, y_setpoint, y0)
  ...
  #rank(ctrb(A, B))
##  To choose eig values below find eig values of A matrix and negative them.
...
function [t,y] = lqr_pendulum(m, g, L, y_setpoint, y0)
  [A,B] = pendulum_AB_matrix(m, g, L);               ## Initialize A and B matrix
  Q = 100*[1 0; 0 1];                   ## Initialise Q matrix ## x1 -> angle pendulum makes x2 -> angular velocity
  R = 1000*[1];                   ## Initialise R 

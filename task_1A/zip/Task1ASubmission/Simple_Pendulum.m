...
function dy = pendulum_dynamics(y, m, L, g, u)
  ...
  cos_theta = cos(y(1));
  dy(1,1) = y(2);                               
  dy(2,1) = -(g*sin_theta)/L + (1/(m*L^2))*u;## This is -g/l(sin(x1)) + (1/ml^2)*u, NOTE: add . in front of arithmatic operators to represent scalar multiplication.    
endfunction
...
function [t,y] = pole_place_pendulum(m, g, L, y_setpoint, y0)
  ...
  rank(ctrb(A, B))
  eigs = [-3.13 -3.13];                             ## Initialise desired eigenvalues
